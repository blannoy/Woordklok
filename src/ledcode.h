...
void ledShowClockface() {
  ...
  switch (config.ledMode) {
    case single: color = valueToColor(config.singleColorHue); break;
    case hourly: color = valueToColor(config.hourlyColors[dateHours]); break;
    case rainbow: color = valueToColor(ledRandomHue); break;
  }
    ...
    if (word.isActive(dateHours % 12, dateMinutes)) {
      for (int pos : word.leds) {
        if (config.ledMode != words) {
          ...
          color = valueToColor(config.wordColors[word.colorCodeInTable]);
          setColor(pos, color);
        }
      }
...
byte calculateTimeBrightness() {
  if (dateHours > config.brightnessStartHour && dateHours < config.brightnessEndHour) {
    return config.maxBrightness;
  } else if (dateHours < config.brightnessStartHour || dateHours > config.brightnessEndHour) {
    return config.minBrightness;
  } else if (dateHours == config.brightnessStartHour) {
    return constrain(
      map(dateMinutes, 0, 29, config.minBrightness, config.maxBrightness),
      config.minBrightness, config.maxBrightness);
  } else if (dateHours == config.brightnessEndHour) {
    return constrain(
      map(dateMinutes, 0, 29, config.maxBrightness, config.minBrightness),
      config.minBrightness, config.maxBrightness);
  }
  return config.minBrightness;
}
...
byte calcBrightness() {
  ...
  switch (config.brightnessMode) {
    case fixedBrightness: brightness = config.maxBrightness; break;
    case ldrBrightness:
      brightness = constrain(
        map(LDRvalue, config.ldrDark, config.ldrBright, config.minBrightness, config.maxBrightness),
        config.minBrightness, config.maxBrightness);
      break;
    case timeBrightness: brightness = calculateTimeBrightness(); break;
  }
  return brightness;
}
...
HslColor valueToColor(int colorValue){
  if (colorValue == -1){
    return HslColor(white);
  } else {
    return HslColor(colorValue / 255.0, 1.0f, 0.5f);
  }
}