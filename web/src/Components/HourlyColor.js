  ...
  return (
    <div>
      ...
      { [...Array(24)].map((value,index)=> {
        ...
        return <div key={colorIndex} style={{ display: "flex", justifyContent: "left" }}><div style={{ width:"10%", fontWeight: "bold", textAlign: "center"}}>{String(index)+":00"} </div><div><ColorPicker id={colorIndex} currentVal={colorList[index]} onColorChoice={transformConfig} /></div></div>
      })}
      </div> 
    </div>
  );
